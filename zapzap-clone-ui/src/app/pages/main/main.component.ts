...
export class MainComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  sendMessage() {
    if (this.messageContent) {
      const messageRequest: MessageRequest = {
        chatId: this.selectedChat.id ?this.selectedChat.id:'',
    ...
    if (file !== null) {
      ...
      reader.onload = () => {
        if (reader.result) {
          ...
          this.messageService.postMessageWithFile({
  ...
  private setMessagesToSeen() {
    this.messageService.patchStatusChange({
      chatId: this.selectedChat.id as string,
      status: "SEEN"    
  ...
  private getAllChatMessages(chatId: string) {
    this.messageService.getMessagesByChat({
      chatId
    }).subscribe({
      next: (messages) => {
  ...
  private handleNotification(notification: Notification) {
    ...
    if (this.selectedChat && this.selectedChat.id === notification.chatId) {
      ...
      if (destChat && notification.type !== 'SEEN') {
        ...
        destChat.unreadMessagesCount! += 1;
